...
class IndexController extends Controller
{
    public function __invoke(Request $request)
    {
        ...
        $query = DB::table('store_cart')
            ...
            ->leftJoin('store_product_pricing', function($join) {
                $join->on('store_cart.pricing_option_id', '=', 'store_product_pricing.id')
                     ->where('store_cart.item_type', '=', 'product');
            })
            ->leftJoin('store_service_pricing', function($join) {
                $join->on('store_cart.pricing_option_id', '=', 'store_service_pricing.id')
                     ->where('store_cart.item_type', '=', 'service');
            })
            ...
            ->select(
                ...
                // 가격 옵션 정보
                'store_product_pricing.name as product_pricing_name',
                'store_product_pricing.price as product_pricing_price',
                'store_service_pricing.name as service_pricing_name',
                'store_service_pricing.price as service_pricing_price',
                // 사용자 정보
        ...
        return view('jiny-store::admin.cart.index', [
            'cartItems' => $cartItems,
            'stats' => $stats,
            'insights' => $insights,
